...
const toComp = (node, idx) => {
    if (node.nodeType === 1) {
        ...
        let children = Array.prototype.slice
            ...
            .map((child, idxChild) => toComp(child, idxChild));
...
export const SvgWrapper = initialProps => {
    ...
    const viewBox = doc.documentElement.attributes.viewBox
        ? doc.documentElement.attributes.viewBox.value
        : null;
    const fill = doc.documentElement.attributes.fill
        ? doc.documentElement.attributes.fill.value
        : null;
